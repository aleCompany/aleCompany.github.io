...
|변수타입|의미|비고|
|-|-|-|
|Numeric|실수|연산가능 ,as.factor() |
|Integer|정수|연산가능, as.integer()|
|Complex|복소수|연산가능, as.complex()|
|Character|문자|연산불능, as.complex|
|Logical|논리|연산불능, as.logical|
|Factor|범주|연산불능 ,as.factor() |
|Date|날짜|연산불능, as.Date|

### 데이터구조
|구조|차원|비고|
|-|-|-|
|Vector|1|한가지변수타입|
|Dataframe|2|다양한변수타입 -> 대부분 데이터 분석에 사용|
|Matirx|2|한가지변수타입|
|Array|다차원|동일한구조, 2차원이상Matrix|
|List|다차원|서로다른구조, 2차원이상의Dataframe -> Web Data 사용|
