...
```{python}
df8.stack()   #  colum이 열로 치환 , .unstack()  반대
```
```
first  second   
bar    one     A   -0.633631
...
* Date times (Timestamp): 특정 날짜와 시간을 설정 (datetime.datetime 과 유사)
* Time deltas (Timedelta): 시간 간격 설정 및 계산 (datetime.timedelta 와 유사)
* Time spans (Period): 주기(frequency)에 따라 시간 영역을 설정
* Date offsets (DateOffset): 상대적 시간 간격 설정
